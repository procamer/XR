...
namespace XR
{
    public class Renderer
    {
        ...
        public Renderer(Model scene)
        {
            _scene = scene;
            string vertexDefines = string.Format("uniform mat4 boneTransform[{0}];", scene.Animator.MaxBoneCount);
        ...
        public void Render(Matrix4 view, Matrix4 perspective)
        {
            ...
            for (int i = 0; i < _scene._meshes.Count; i++)
            {
                ...
                Matrix4 model = _scene._modelMatrix;
                shader.SetMat4("modelMatrix", model);

                ApplyMaterial(shader, _scene.Raw.Materials[_scene._meshes[i].MaterialIndex]);

                if (_scene.Raw.HasAnimations && _scene.Animator.ActiveAnimation > -1)
                {
                    shader.SetInt("hasAnimations", 1);
                    _scene.Animator._bones = _scene._meshes[i].boneTransforms;
                    _scene.Animator.UpdateAnimation();
                    for (int j = 0; j < _scene.Animator._bones.Count; j++)
                        shader.SetMat4("boneTransform[" + j + "]", _scene.Animator._bones[j].Transformation);
                }
                else
                {
                    shader.SetInt("hasAnimations", 0);
